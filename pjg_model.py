...
# =============================================================================
# 1b. Gymnasium 환경 (pjg2_model 등에서 import)
# =============================================================================

class OmokEnv(gym.Env):
    """Gymnasium: Multi-channel 관측, action_mask, 착수 순서 GUI."""
    ...
    def step(self, action):
        ...
        self.move_order[r, c] = self.move_count
...
# =============================================================================
# 8. Arena 연동용 Agent (gomoku.py main에서 1턴씩 호출)
# =============================================================================
class PJGModel2:
    """
    gomoku.py 같은 Arena 루프에서 바로 사용할 수 있는 1턴 행동 에이전트.
    - select_action(state)
    - select_action(state, player_id=2)
    둘 다 지원한다.
    """
    ...
    def __init__(
        ...
        if not os.path.isfile(resolved_path):
            resolved_path = os.path.join(model_dir, "gomoku_ppo_final.pth")
        self.model_path = resolved_path

        self.board_size = 15
        self.model = GomokuCNN(board_size=self.board_size, n_actions=self.board_size ** 2).to(self.device)
        if os.path.isfile(self.model_path):
            load_model(self.model, self.model_path, self.device)
        else:
            raise FileNotFoundError(
                f"PJGModel2: model file not found at '{self.model_path}'. "
                "훈련 후 .pth 파일 경로를 확인하세요."
            )
        self.model.eval()

    def _build_obs_from_state(self, state: np.ndarray, player_id: int = 1) -> np.ndarray:
        """
        state가 0/1/2 보드일 때, player_id 관점의 (3,15,15) 관측으로 변환.
        """
        board = np.asarray(state, dtype=np.int8)
        my_stones = (board == player_id).astype(np.float32)
        opp_stones = (board == (3 - player_id)).astype(np.float32)
        turn_channel = np.full((self.board_size, self.board_size), float(player_id == 1), dtype=np.float32)
        return np.stack([my_stones, opp_stones, turn_channel], axis=0)

    def select_action(self, state, player_id: int = 1):
        """
        Arena 루프에서 한 수를 반환.
        - gomoku.py처럼 state를 inverted_state(내 돌=1, 상대 돌=2)로 넘기면 player_id 기본값(1) 사용.
        - raw state를 넘긴다면 player_id를 현재 턴 플레이어로 지정해도 됨.
        """
        board = np.asarray(state, dtype=np.int8)
        if board.shape != (self.board_size, self.board_size):
            raise ValueError(f"PJGModel2 expects state shape {(self.board_size, self.board_size)}, got {board.shape}")
        ...
        # 우선 규칙 기반 수를 시도
        rule_action = select_action_with_rules(board.copy(), player_id, mask)
        if rule_action is not None:
            return int(rule_action)

        # 규칙 수가 없으면 학습된 정책으로 착수
        obs = self._build_obs_from_state(board, player_id=player_id)
        ...
        action, _ = self.model.get_action(obs_t, mask_t, deterministic=True)
        return int(action)
